import { HandlerFunc } from "https://deno.land/x/abc@v1.0.0-rc2/mod.ts";
import db from "../db/db.ts";
import { ErrorHandler } from "../errohandler/errorhandler.ts";
...
export const createEmployee: HandlerFunc = async (c) => {
  try {
    ...
    throw new error.ErrorHandler(error.message, error.status || 500);
  }
};


export const fetchAllEmployees: HandlerFunc = async (c) => {
    try {
  ...
  export const fetchOneEmployee: HandlerFunc = async (c) => {
    try {
  ...
  export const updateEmployee: HandlerFunc = async (c) => {
    try {
      const { id } = c.params as { id: string };
      if (c.request.headers.get("content-type") !== "application/json") {
        throw new ErrorHandler("Invalid body", 422);
  ...
  export const deleteEmployee: HandlerFunc = async (c) => {
    try {
      const { id } = c.params as { id: string };
